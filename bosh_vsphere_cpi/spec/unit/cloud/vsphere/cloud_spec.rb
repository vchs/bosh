...
vcenters:
  - host: 10.146.19.1
    ...
    shared_context 'base' do
      ...
      let(:datastore) do
        datastore = double('datastore1')
        datastore.stub(:name) { 'datastore1' }
        datastore
      end
      ...
      let(:snapshot) do
        snapshot = double('vm snapshot')
        snapshot.stub(:current_snapshot) { double('snapshot') }
        snapshot
      end
      ...
      let(:cluster1) do
        cluster1 = double('cluster1')
        cluster1.stub(:mob) { cluster }
      ...
      let(:datastore1) do
        ...
        datastore1.stub(:name) { 'datastore1' }
        datastore1
      end
      ...
      let(:virtual_disk) do
        ...
        virtual_disk = VimSdk::Vim::Vm::Device::VirtualDisk.new(controller_key: vdisk_controller_key)
        virtual_disk
      end
